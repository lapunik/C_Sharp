...
namespace DKEY
{
    ...
    {
        public int halfID = 0;
        public byte[] rcpData = { 0, 0, 0, 0, 0 };

        public RcpData(int inTime, byte[] inputArr, int length)
            : base(inTime, inputArr, length) { }//constructor inherited from parent class 

        public override bool CheckMsg()
        {
            //ToDo implementation for RCP Data/Information
            fc = rxArr[2] >> 4;
            halfID = rxArr[1] | (rxArr[0] << 8);
            if (fc == 0x0C)
            {
    ...
    class AntiColl : Msg
    {
        ...
        public override bool CheckMsg()  // check all posible FC , also CKT FC  even if it has a different baudrate
        {
            //check crc4
            ...
            {
                // CRC with the FC 0xE or AC with a wrong CRC?
                crc4Value = Check_AC_crc4(this.rxArr, 4, 0xe);
                if (crc4Value == crc4Received)//Check_AC_crc4(outArr, 5, 0x9) == (outArr[4] >> 4))
                {
                    fc = 0xe;
                    SetID(rxArr, 0);
                ...
                {
                    // CRC with the FC 0xA -> CKT LF Received
                    crc4Value = Check_AC_crc4(this.rxArr, 4, 0xa);
                    if (crc4Value == crc4Received)//Check_AC_crc4(outArr, 5, 0x9) == (outArr[4] >> 4))
                    {
                        fc = 0xa;
                        SetID(rxArr, 0);
                    ...
                    {
                        // CRC with the FC 0xB -> CKT LF not received
                        crc4Value = Check_AC_crc4(this.rxArr, 4, 0xb);
                        if (crc4Value == crc4Received)//Check_AC_crc4(outArr, 5, 0x9) == (outArr[4] >> 4))
                        {
                            fc = 0xb;
                            SetID(rxArr, 0);
    ...
    class AntiCollCkt : Msg
    {
        public AntiCollCkt(int inTime, byte[] inputArr, int length)
             : base(inTime, inputArr, length) { } //constructor inherited from parent class 
        ...
        {
            //check crc4
            int crc4Value = Check_AC_crc4(this.rxArr, 4, 0xa);  // FC == A IDG received CKT WUP
            int crc4Received = rxArr[4] >> 4;
            if (crc4Value == crc4Received)//Check_AC_crc4(outArr, 5, 0x9) == (outArr[4] >> 4))
            {
                fc = 0xA;
            ...
            {
                // CRC with the FC 0xD or AC with a wrong CRC?
                crc4Value = Check_AC_crc4(this.rxArr, 4, 0xd); // FC == D IDG did not receive CKT WUP
                if (crc4Value == crc4Received)//Check_AC_crc4(outArr, 5, 0x9) == (outArr[4] >> 4))
                {
                    fc = 0xd;
                    SetID(rxArr, 0);
